...
def main():
    ...
    with open(os.path.join(home_directory, 'extracted_ocr\\Lib_Co_Boston_1824.html'), 'r') as f:
        contents = f.read()
        ...
        for index, page in enumerate(file.find_all('page')):
            ...
            for word in words:
                ...
                if re.search("[0-9]", word.text) and re.search("(I|O|l)", word.text):
                    continue
                # If it starts with a letter and contains a number after the letter, it's garbage.
                # That pattern doesn't occur in English.
                if re.match("[A-Za-z]+[0-9]+", word.text):
                    continue
    ...
    with open(os.path.join(home_directory, 'replication\\Lib_Co_Boston_1824.csv'), 'wb+') as outfile:
